...
class Intervals(LayoutBase):
    ...
    def get_layout(cls):
        ...
        sleep_modifier = (
            intervals['gui.sleep_modifier'] if 'gui.sleep_modifier' in intervals else '')
        loop_break = (
            intervals['gui.loop_break_seconds'] if 'gui.loop_break_seconds' in intervals else '')
        # instant = (
    ...
    def save_intervals(values):
        settings = {
            'gui.sleep_modifier': values['gui.sleep_modifier'],
            'gui.loop_break_seconds': values['gui.loop_break_seconds'],
        }
        JsonData.dump_json(settings, 'gui_intervals.json', pretty=True)
    ...
    def update_gui(cls, window, event, values):
        ...
        if event == 'save_intervals':
            cls.save_intervals(values)
